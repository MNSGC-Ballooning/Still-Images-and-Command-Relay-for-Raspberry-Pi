...
class xbeeThread(threading.Thread):
    ...
    def run(self):
        global xbeeReceived, xbeeToSend, xPort, xBaud, xTimeout
        xbee = serial.Serial(port = xPort, baudrate = xBaud, timeout = xTimeout)
        while True:
            try:
                line = xbee.readline()
                if line not in xbeeReceived:
                    xbeeReceived.append(line)
                print(xbeeReceived)
                print(xbeeToSend)
                for each in xbeeToSend:
                    xbee.write(each)
                xbeeToSend = []
            except:
                resetFlag = True
...
sideThread = xbeeThread("Thread -1")
sideThread.start()
...
##    print "XBee: " + xLine
##    ser.write(xLine)
    xbeeToSend.append(command)
    for each in xbeeReceived:
        ser.write(each)
    xbeeReceived = []
    if resetFlag == True:
        sideThread.start()
