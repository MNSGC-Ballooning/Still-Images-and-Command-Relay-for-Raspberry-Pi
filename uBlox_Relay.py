...
Edit June 12 2017: Revised version of code for use with Ublox gps, allowing configuration of gps to air mode
"""
...
class GPSThread(threading.Thread):
    ...
    def setAirborne(self):
        airMode = bytes([0xB5,0x62,0x06,0x24,0x24,0x00,0xFF,0xFF,0x06,0x02,0x00,0x00,0x00,0x00,
                   0x10,0x27,0x00,0x00,0x05,0x00,0xFA,0x00,0xFA,0x00,0x64,0x00,0x2C,0x01,0x00,0x00,0x00,
                   0x00,0x10,0x27,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x4C,0xBA])
        acknowledge = bytes([0xB5, 0x62, 0x05, 0x01, 0x02, 0x00, 0x06, 0x24, 0x32, 0x5B])
        rcv = "received"
        while (rcv != ""):
            rcv = self.gpsSer.readline()
        self.gpsSer.write(airMode)
        time.sleep(.1)
        response = self.gpsSer.read(len(acknowledge))
    ...
    def run(self):
        ...
        for n in range(3):
            if(self.setAirborne()):
                if self.loggingGPS:
                    self.gpsQ.put("Air mode successfully set.")
                    try:
                        f = open(folder+"gpslog.txt","a")
                        f.write("Air mode successfully set.")
                        f.close()
                    except Exception, e:
                ...
                break
            if(n==2):
                self.gpsQ.put("WARNING: Failed to set ot air mode (3 attempts).")
                if self.loggingGPS:
                    try:
                        f = open(folder+"gpslog.txt","a")
                        f.write("WARNING: Failed to set to air mode (3 attempts). Altitude data may be unreliable.")
                        f.close()
                    except Exception, e:
                        print("Error logging GPS")
                        self.exceptionsQ.put(str(e))
